...
customer scale so that scan reads scaling in ``nc`` actually scale the read
work. Each subtree is a Portfolio → Customer → Asset tree with 6 series per
asset (1 forecast / OVERLAPPING + 5 flat / FLAT).
...
Forecast parquets are generated per scale, each using the series_ids of that
scale's subtree. Flat seed data is only populated under the max scale (where
narrow-read and cross-retention benchmarks point).

The node tree is inserted directly via SQL rather than through the EDM
serialization path — the benchmark isn't measuring EDM serialization, it's
measuring DB I/O, and direct SQL is faster at these scales.
"""
...
def register_series_for(edb: EnergyDBClient, asset_ids: dict[tuple[str, str], int], root_label: str) -> pl.DataFrame:
    """Register 6 series per asset (1 OVERLAPPING forecast + 5 FLAT flats)."""
    rows: list[tuple[str, str, str, str, int, bool]] = []
    with edb._pool.connection() as conn:
        for (customer, asset), node_id in asset_ids.items():
            ...
            rows.append((root_label, customer, asset, FORECAST_SERIES_NAME, sid, True))
            for name in FLAT_SERIES_NAMES:
                ...
                rows.append((root_label, customer, asset, name, sid, False))
        conn.commit()
    ...
    return pl.DataFrame(
        {
            ...
            "is_overlapping": [r[5] for r in rows],
        },
        schema={
            "root": pl.Utf8,
            "customer": pl.Utf8,
...
def generate_forecast(id_map: pl.DataFrame, num_customers: int) -> pl.DataFrame:
    ...
    sub = id_map.filter(
        (pl.col("root") == root_name(num_customers)) & (pl.col("series_name") == FORECAST_SERIES_NAME)
    ).sort(["customer", "asset"])
...
def generate_flat_seed(id_map: pl.DataFrame) -> pl.DataFrame:
    ...
    sub = id_map.filter((pl.col("root") == MAX_ROOT) & ~pl.col("is_overlapping")).sort(
        ["customer", "asset", "series_name"]
    )
    sids = sub["series_id"].to_numpy()
...
def main() -> None:
    ...
    for nc in CUSTOMER_SCALES:
        ...
        print(f"{len(asset_ids):>6,} assets, {len(part):>6,} series in {time.perf_counter() - t0:.1f}s")

    id_map = pl.concat(id_map_parts)
    id_map.write_parquet(SERIES_IDS_PATH)
    print(f"  id map → {SERIES_IDS_PATH.name} ({len(id_map):,} rows, {_file_size(SERIES_IDS_PATH)})")
    ...
    df.write_parquet(path)
    elapsed = time.perf_counter() - t0
    print(f"  flat seed  rows={len(df):<12,} (under {MAX_ROOT}) → {path.name} ({_file_size(path)}) in {elapsed:.1f}s")
